...
//////                    std::cout << "Read line: " << element << "   " << x << "    " << y << "    " << z  << endl;
        		    if(element == 1)
                    {
                        ++number_of_gas; // sums the number of gas atoms
                        out_file << "// Atom number " <<  number_of_atoms << " Gas atom number " << number_of_gas  << endl;
                        out_file << "sphere { <" << x << "," << y << "," << z << ">," << Rgas << " pigment { rgbt <1,0,0,0>} finish { phong 0.7 phong_size 20 }}"  << endl;
                        
                        out_framexyz << element << "   " << x << "    " << y << "    " << z << endl;
                    }
                    
                    if(element == 2)
                    {
                        ++number_of_liquid; // sums the number of gas atoms
                        out_file << "// Atom number " <<  number_of_atoms << " Liquid atom number " << number_of_liquid  << endl;
                        out_file << "sphere { <" << x << "," << y << "," << z << ">," << Rliquid << " pigment { rgbt <0,0,1,0>} finish { phong 0.7 phong_size 20 }}" << endl; //0.85 transparency
